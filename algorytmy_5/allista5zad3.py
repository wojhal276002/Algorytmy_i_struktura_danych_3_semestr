...
class ChainHashMap(HashMapBase):
    ...
    def _bucket_setitem(self, k):
        ...
        mix_1 = (3*k+5)%11
        mix_1_do_2 = (3*k+5)%11
        mix_2 = 7 - (mix_1_do_2 % 7)
        while self._table[mix_1] is not None:# bucket is new to the table
            if self._table[mix_1] is not None:
                mix_1 = (mix_1+mix_2) % len(self._table)
        self._table[mix_1] = k
        if self._lista == None:
            self._lista = []
        self._lista.append([k,mix_1])
